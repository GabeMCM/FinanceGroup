...
const splitModes: SplitMode[] = ["equal", "proportional", "fixed_percent", "individual"];
...
export async function POST(request: NextRequest) {
  ...
  if (membersError || !members?.length) {
    return NextResponse.json({ error: membersError?.message ?? "Grupo sem membros." }, { status: 422 });
  }
  ...
  return NextResponse.json({ data: { ...item, participants, payments } }, { status: 201 });
}
